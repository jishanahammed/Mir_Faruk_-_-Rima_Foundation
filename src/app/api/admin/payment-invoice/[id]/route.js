...
import { getAdminDonorPaymentHistoryById } from "@/lib/api/admin-donor-payment-history-service";
...
export async function GET(_request, context) {
  ...
  try {
    const payment = await getAdminDonorPaymentHistoryById(id);

    if (!payment) {
      return new Response("Payment history was not found.", { status: 404 });
    }
