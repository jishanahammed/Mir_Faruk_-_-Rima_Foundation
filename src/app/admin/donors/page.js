import { DonorListTable } from "@/components/admin/donor-list-table";
...
export const metadata = {
  title: "Admin Donor List | Mir Faruk & Rima Foundation",
};
...
export default async function AdminDonorsPage({ searchParams }) {
  ...
  try {
    donors = await getAdminDonorList(filters);
  } catch (error) {
    ...
    <div className="space-y-5 xl:space-y-6">
      {errorMessage ? (
        <section className="rounded-[24px] border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-700 sm:px-5 sm:py-4">
          <strong className="block font-semibold">Unable to load donor list</strong>
          {errorMessage}
        </section>
      ) : null}

      <DonorListTable donors={donors} filters={filters} />
    </div>
  );
}
