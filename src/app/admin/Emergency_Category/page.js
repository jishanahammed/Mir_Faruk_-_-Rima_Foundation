...
import { EmergencyCategoryTable } from "@/components/admin/Emergency_Category/emergency-category-table";
import { EmergencyCategoryModal } from "@/components/admin/Emergency_Category/emergency-category-modal";
import {
  createEmergencyCategoryAction,
  updateEmergencyCategoryAction,
  updateEmergencyCategoryStatusAction,
  deleteEmergencyCategoryAction,
  uploadEmergencyCategoryImageAction,
  deleteEmergencyCategoryImageAction,
} from "./actions";

export const metadata = {
  title: "Emergency Categories | Admin | Mir Faruk & Rima Foundation",
};
...
export default async function AdminEmergencyCategoryPage({ searchParams }) {
  ...
  try {
    allCategories = await getAdminEmergencyCategories();
  } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-bold tracking-widest text-cyan-700 uppercase">Emergency</p>
          <h1 className="mt-1 text-2xl font-extrabold text-slate-900">Emergency Categories</h1>
          <p className="mt-1 text-sm text-slate-500">
            Manage categories used to organise emergency donation campaigns.
          </p>
        </div>
      ...
      <section className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-xl shadow-slate-900/5">
        <div className="border-b border-slate-100 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.15),transparent_40%),linear-gradient(135deg,#f8fafc,#f0fdf9)] px-6 py-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              href="/admin/Emergency_Category?add=1"
        ...
        <EmergencyCategoryTable
          data={paginationData}
          search={search}
          updateStatusAction={updateEmergencyCategoryStatusAction}
          deleteAction={deleteEmergencyCategoryAction}
        />
      </section>
      ...
      <EmergencyCategoryModal
        ...
        createAction={createEmergencyCategoryAction}
        updateAction={updateEmergencyCategoryAction}
        uploadAction={uploadEmergencyCategoryImageAction}
        deleteImageAction={deleteEmergencyCategoryImageAction}
      />
    </div>
  );
}
