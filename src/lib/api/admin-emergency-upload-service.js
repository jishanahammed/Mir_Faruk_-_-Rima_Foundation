...
export async function uploadEmergencyCampaignImages(webFormData) {
  ...
  const res = await authApiClient.post(
    "EmergencyCampaigns/upload-images",
    axiosForm,
    {
      headers: {
...
export async function deleteEmergencyCampaignImage(imagePath) {
  const token = await getToken();
  await authApiClient.delete("EmergencyCampaigns/delete-image", {
