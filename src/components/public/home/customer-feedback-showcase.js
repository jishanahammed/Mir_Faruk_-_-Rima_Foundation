    ...
    <div className="flex items-center gap-0.5" aria-label={`${rating} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          viewBox="0 0 24 24"
          className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${star <= rating ? "fill-amber-400" : "fill-slate-200"}`}
        >
...
function FeedbackCard({ item }) {
  return (
    <article className="flex h-full flex-col rounded-xl border border-slate-200 bg-white p-3 shadow-sm shadow-slate-900/5 transition hover:-translate-y-0.5 hover:shadow-md hover:shadow-cyan-900/10 sm:rounded-2xl sm:p-4">
      <Stars rating={item.rating} />
      <p className="mt-2 flex-1 text-[11px] leading-4 text-slate-700 line-clamp-3 sm:mt-2.5 sm:text-xs sm:leading-5 sm:line-clamp-4">
        &ldquo;{item.message}&rdquo;
      </p>
      <div className="mt-2.5 flex items-center gap-2 border-t border-slate-100 pt-2.5 sm:mt-3 sm:gap-2.5 sm:pt-3">
          ...
          <div className="mt-8 flex items-center justify-center gap-2 sm:mt-10">
            ...
            {Array.from({ length: totalPages }, (_, index) => index + 1).map((pageNumber) => (
              <PaginationButton
                ...
                className={`inline-flex h-9 min-w-9 items-center justify-center rounded-xl border px-3 text-xs font-bold transition ${
                  pageNumber === page
