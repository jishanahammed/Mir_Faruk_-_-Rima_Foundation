...
function CycleCard({ icon, title, description, highlight }) {
  return (
    <div
      className={`flex w-full max-w-xs flex-col items-center gap-1.5 rounded-3xl border px-4 py-4 text-center shadow-sm lg:w-40 ${
        highlight
    ...
    >
      ...
      <p
        className={`text-[0.68rem] leading-5 ${
        ...
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-[0_18px_60px_rgba(6,95,70,0.06)] sm:p-8">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            {di.example.phases.map((phase, index) => (
              ...
              >
                <span
                  className={`inline-flex rounded-full px-3.5 py-1 text-xs font-bold text-white ${
                    ["bg-emerald-700", "bg-sky-700", "bg-orange-500"][index] ??
                    "bg-emerald-700"
                  }`}
                >
                  {phase.year}
