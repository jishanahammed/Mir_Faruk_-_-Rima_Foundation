...
function GoatHerd({ mothers, kids }) {
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex flex-wrap items-center justify-center gap-1">
        {Array.from({ length: mothers }).map((_, index) => (
          <GoatGlyph key={`m-${index}`} className="text-3xl sm:text-4xl" />
        ))}
      </div>
      {kids > 0 ? (
        <div className="flex max-w-[22rem] flex-wrap items-center justify-center gap-x-1 gap-y-0.5">
          {Array.from({ length: kids }).map((_, index) => (
            <GoatGlyph key={`k-${index}`} className="text-base sm:text-lg" />
          ))}
        </div>
      ) : null}
    </div>
            ...
            <tbody>
              {stages.map((stage) => (
                ...
                >
                  ...
                  <td className="border-r border-emerald-100 px-4 py-6">
                    <GoatHerd mothers={stage.mothers} kids={stage.kids} />
                  </td>
