...
import { VideoSpeechImageUploader } from "./video-speech-image-uploader";
...
export function VideoSpeechModal({
  params,
  items,
  createAction,
  updateAction,
  ...
  const editItem = editId ? items.find((t) => t.id === editId) : null;
  ...
  function close() {
    router.push("/admin/video-speech");
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 max-h-[90vh] overflow-y-auto">
        <div className="flex items-start justify-between border-b border-slate-100 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.12),transparent_40%),linear-gradient(135deg,#f8fafc,#f0fdf9)] px-6 py-5">
          <div>
            ...
            <h3 className="text-base font-bold text-slate-900">
              {isEdit ? `Edit: ${editItem.nameEn}` : "Add Video Speech"}
            </h3>
          </div>
          <button
            type="button"
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-5 p-6">
          <VideoSpeechImageUploader
            name="backgroundImageUrl"
            label="Background Image"
            uploadAction={uploadImageAction}
            deleteAction={deleteImageAction}
            existingImagePath={editItem?.backgroundImageUrl}
          />

          <div>
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">Name</p>
          ...
          <div>
            <p className="mb-3 text-xs font-bold uppercase tracking-wider text-slate-400">Role</p>
            <div className="grid gap-4 sm:grid-cols-3">
              <Field name="roleEn" label="English" required placeholder="Role in English" defaultValue={editItem?.roleEn} />
              <Field name="roleBn" label="বাংলা" required placeholder="বাংলায় পদবী" defaultValue={editItem?.roleBn} />
              <Field name="roleDk" label="Dansk" placeholder="Rolle på dansk" defaultValue={editItem?.roleDk} />
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div>
              ...
              <input
                name="videoId"
                required
                defaultValue={editItem?.videoId}
                placeholder="e.g. fjmA9lTguvc"
            ...
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-slate-500">Sort Order</label>
              <input
                name="sortOrder"
                type="number"
                min="0"
            ...
            <div>
              ...
              <select
                name="isPublished"
                defaultValue={editItem ? String(editItem.isPublished) : "false"}
          ...
          <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
            ...
            >
              {isPending && <Spinner />}
              {isPending ? "Saving…" : isEdit ? "Save Changes" : "Add Video Speech"}
            </button>
          </div>
        </form>
      </div>
