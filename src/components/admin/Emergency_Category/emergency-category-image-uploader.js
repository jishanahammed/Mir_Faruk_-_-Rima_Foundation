...
export function EmergencyCategoryImageUploader({ uploadAction, deleteAction, existingImagePath }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <input type="hidden" name="imageUrl" value={saved ?? ""} />

      {preview ? (
        <div className="group relative h-32 w-32 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-sm">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={preview.url} alt="Category" className="h-full w-full object-cover" />
          {preview.path === null && (
        ...
        >
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-slate-700">
              {isPending ? "Uploading…" : "Click or drag an image here"}
            </p>
