...
export function ProjectImageUploader({ uploadAction, deleteAction, existingImages = [] }) {
        ...
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 lg:grid-cols-5">
          {previews.map((item, idx) => (
            <div key={idx} className="group relative aspect-square overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-sm">
              ...
              <img src={item.url} alt={`Project image ${idx + 1}`} className="h-full w-full object-cover transition-transform duration-200 group-hover:scale-105" />
              {item.path === null && (
