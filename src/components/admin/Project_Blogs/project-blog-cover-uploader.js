...
export function ProjectBlogCoverUploader({ uploadAction, deleteAction, existingImage }) {
  const inputRef = useRef(null);
  const [isPending, startTransition] = useTransition();

  const [saved, setSaved] = useState(existingImage ?? null);
  const [preview, setPreview] = useState(
    existingImage ? { path: existingImage, url: buildPreviewUrl(existingImage) } : null
  );
  ...
  function handleFile(file) {
    ...
    setPreview({ path: null, url: localUrl, file });
    ...
    startTransition(async () => {
      try {
        ...
        setPreview(url ? { path: url, url: buildPreviewUrl(url) } : null);
        setSaved(url);
      } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <input type="hidden" name="coverImage" value={saved ?? ""} />
      ...
      {!preview ? (
        ...
        >
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-slate-700">
              {isPending ? "Uploading…" : "Click or drag a cover image here"}
            </p>
            <p className="mt-0.5 text-xs text-slate-400">JPG, PNG, GIF, WebP · Max 5 MB</p>
          </div>
          <input
            ref={inputRef}
            type="file"
        ...
        <div className="group relative aspect-video w-full max-w-xs overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-sm">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={preview.url} alt="Cover" className="h-full w-full object-cover" />
          {preview.path === null && (
      ...
      {error && (
        <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3">
          ...
          <button type="button" onClick={() => setError(null)} className="mt-1 text-[10px] font-bold text-rose-500 underline">
            Dismiss
          </button>
        </div>
      )}
