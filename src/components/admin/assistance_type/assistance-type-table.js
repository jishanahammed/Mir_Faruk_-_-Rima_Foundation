...
import { AssistanceTypeDeleteButton } from "./assistance-type-delete-button";
import { AssistanceTypeStatusToggle } from "./assistance-type-status-toggle";
...
function Pagination({ pageNumber, totalPages, pageSize, search }) {
  function href(p) {
    ...
    return `/admin/assistance-type?${sp.toString()}`;
  }
...
export function AssistanceTypeTable({ data, search, updateStatusAction, deleteAction }) {
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3 border-b border-slate-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
        <form method="get" action="/admin/assistance-type" className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <form method="get" action="/admin/assistance-type">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-700">No assistance types found</p>
          <p className="mt-1 text-xs text-slate-400">
            {search ? "Try a different search term." : "Add your first assistance type to get started."}
          </p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {items.map((type, idx) => (
                <tr key={type.id} className="group transition hover:bg-cyan-50/40">
                  <td className="px-4 py-3 text-xs font-semibold text-slate-400">
                    {(pageNumber - 1) * pageSize + idx + 1}
                  </td>
                  <td className="px-4 py-3">
                    <div className="font-semibold text-slate-900">{type.nameEn}</div>
                    {type.descriptionEn && (
                      <div className="mt-0.5 line-clamp-1 text-xs text-slate-400">{type.descriptionEn}</div>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-slate-700">{type.nameBn}</div>
                    {type.descriptionBn && (
                      <div className="mt-0.5 line-clamp-1 text-xs text-slate-400">{type.descriptionBn}</div>
                    )}
                  </td>
                  <td className="px-4 py-3 text-slate-500">{type.nameDk ?? "—"}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/admin/assistance-type?edit=${type.id}`}
